...
local function openMenu(data, searchEnabled)
    local search = Config.search
    if search == "custom" then 
        if searchEnabled then search = true
        else search = false end
    end
...
RegisterNetEvent('qb-menu:client:openMenu', function(data, searchEnabled)
    openMenu(data, searchEnabled)
end)
