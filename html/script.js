...
const openMenu = (data = null, searchEnabled, darkModeEnabled) => {
  let html = "";
  let headerHtml = "";
  let hasVisibleButtons = false;

  const container = $(".menu-buttons");
    ...
    data.forEach((item, index) => {
      if (!item.hidden) {
        ...
        if (!isMenuHeader) {
          ...
          headerHtml = getButtonRender(header, message, index, isMenuHeader, isDisabled, icon);
        }
  ...
  $("#buttons").html(html);
  $(".menu-header").html(headerHtml);
  filterButtons("");
  
  if (searchEnabled) { $("#search-bar").show(); }
...
window.addEventListener("message", (event) => {
    ...
    const searchEnabled = data.search;
    const darkModeEnabled = data.darkMode;
    switch (action) {
        case "OPEN_MENU":
          return openMenu(buttons, searchEnabled, darkModeEnabled);
        case "SHOW_HEADER":
